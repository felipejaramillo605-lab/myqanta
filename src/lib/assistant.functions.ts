...
ACTIVE PROJECTS: ${JSON.stringify(projRes.data ?? [])}`;
    ...
    if (data.attachment && isDocumentAttachment(data.attachment.mime, data.attachment.name)) {
      ...
      if ("error" in parsed) {
        ...
        docBlock = `\n\nATTACHED DOCUMENT "${parsed.name}" (${parsed.kind}${parsed.truncated ? ", truncated" : ""}):\n\`\`\`\n${parsed.text}\n\`\`\`\nRead it carefully. If it contains a financial statement, trial balance or a list of transactions: summarize what you understood, map each item to the org's PUC accounts and use create_draft_journal_entry to create the DRAFT entries (one call per entry, balanced). Never post entries.\n\nMISSING RECORDS FLOW (mandatory): before creating entries from a file, call check_missing_accounts_and_parties with every referenced account code (plus the name you would give it, taken from the file) and every supplier name/NIT. If it reports missing items, DO NOT abort and DO NOT tell the user to go create them manually: list them clearly (code - name) and ask "¿Deseas crearlas ahora para continuar?". Only when the user confirms, call create_puc_accounts and/or create_third_parties with confirmed=true, and then immediately retry create_draft_journal_entry with the SAME data and confirm success. If the user declines, stop and explain they can create them in Finanzas → Plan de cuentas. If create_draft_journal_entry itself returns missing_accounts, follow the same ask → create → retry loop. Never ask twice for the same items in a row.`;
      }
    }
