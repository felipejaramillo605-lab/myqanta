...
import { Plus, Trash2, Check, Flame, CheckCircle2, Circle, Loader2, CalendarDays, Pencil } from "lucide-react";
...
export function TasksPanel() {
  ...
  const save = (v: TaskFormValue) =>
    upsertFn({ data: v })
      .then((row: { id: string } | null | undefined) => {
        refresh();
        toast.success("✓");
        if (!v.id && row?.id) {
          setReminderPrompt({ source_type: "task", source_id: row.id, title: v.title });
        }
      })
            ...
            onSubmit={(v) =>
              createFn({ data: { ...v, cadence: "daily" } })
                .then((row: { id: string } | null | undefined) => {
                  ...
                  toast.success("✓");
                  if (row?.id) {
                    setReminderPrompt({ source_type: "habit", source_id: row.id, title: v.name });
                  }
                })
                .catch((e: Error) => toast.error(e.message))
            }
