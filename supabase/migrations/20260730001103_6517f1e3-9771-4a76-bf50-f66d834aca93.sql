CREATE INDEX IF NOT EXISTS idx_security_events_rate
...
BEGIN
  ...
  -- Caller fingerprint: prefer CDN-provided client IP, then x-forwarded-for.
  BEGIN
  ...
  -- Rate limit: max 8 events per 60s per email, else per caller IP / user.
  ...
  IF recent >= 8 THEN
    RETURN NULL; -- silent drop, same as a disallowed event_type
  END IF;
